...
typedef int (*lora_api_send)(const struct device *dev,
	...
	/* typedef int (*lora_api_send)(const struct device *dev, struct lora_modem_config *config,
			     uint8_t *data, uint32_t data_len); */
...
/* static inline int lora_send(const struct device *dev, struct lora_modem_config *config,
			    uint8_t *data, uint32_t data_len)
{
	const struct lora_driver_api *api =
		(const struct lora_driver_api *)dev->api;
